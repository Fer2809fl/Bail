...
// ========== FUNCIÓN PARA REPARAR SESIÓN CORRUPTA ==========
async function repairCorruptSession(senderKeyStore, senderKeyName) {
    console.log('[GroupCipher] Reparando sesión corrupta para:', senderKeyName?.toString());
    try {
        if (senderKeyStore && senderKeyStore.storeSenderKey) {
            // Forzar eliminación de la clave corrupta
            await senderKeyStore.storeSenderKey(senderKeyName, null);
        }
    } catch (e) {
        console.warn('[GroupCipher] Error reparando:', e.message);
    }
    return true;
}
...
class GroupCipher {
    ...
    // ========== decrypt REPARADO con reintentos ==========
    async decrypt(senderKeyMessageBytes, retryCount = 0) {
        ...
        return await this.queueJob(async () => {
            try {
                ...
                // Si es error de sesión o MAC, reintentar después de reparar
                if ((error.message === 'Bad MAC' || error.message === 'No session found to decrypt message') && retryCount < maxRetries) {
                    console.log(`[GroupCipher] Error: "${error.message}", reparando sesión (${retryCount + 1}/${maxRetries})...`);
                    await repairCorruptSession(this.senderKeyStore, this.senderKeyName);
                    return this.decrypt(senderKeyMessageBytes, retryCount + 1);
                }
                throw error;
            }
        });
