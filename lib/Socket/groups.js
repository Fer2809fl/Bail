...
const GROUP_METADATA_CACHE_MAX_ENTRIES = 256;
export const makeGroupsSocket = (config) => {
    ...
    const groupMetadataCached = async (jid) => {
        ...
        while (groupMetadataCache.size >= GROUP_METADATA_CACHE_MAX_ENTRIES) {
            groupMetadataCache.delete(groupMetadataCache.keys().next().value);
        }
        groupMetadataCache.delete(jid);
        groupMetadataCache.set(jid, { data, expiresAt: now + GROUP_METADATA_CACHE_TTL_MS });
        return data;
    };
    /**
