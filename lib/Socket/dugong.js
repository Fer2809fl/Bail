...
export class Dugong {
  ...
  detectType(content) {
    ...
    if (content.paymentInviteMessage) return "PAYMENT_INVITE";
  ...
  /**
   * Construye una burbuja de "orden de compra" (proto.Message.OrderMessage).
   * Nota: WhatsApp muestra la burbuja con título/thumbnail/monto y un botón
   * "Ver detalles". Según el estado actual del protocolo, la confirmación de
   * pago dentro de esa burbuja (checkout) no está soportada del lado del
   * bot — solo la representación visual del pedido.
   */
  async handleOrder(content, _quoted) {
    const data = content.orderMessage;
    return {
      orderMessage: proto.Message.OrderMessage.fromObject({
        orderId: data.orderId || "",
        thumbnail: data.thumbnail,
        itemCount: data.itemCount || 0,
        status: data.status || 1, // INQUIRY
        surface: data.surface || 1, // CATALOG
        message: data.message,
        orderTitle: data.orderTitle,
        sellerJid: data.sellerJid,
        token: data.token,
        totalAmount1000: data.totalAmount1000 || 0,
        totalCurrencyCode: data.currency || "USD",
        messageVersion: data.messageVersion || 1,
      }),
    };
  }
  /**
   * Construye una burbuja de "invitación de pago" (proto.Message.PaymentInviteMessage).
   * Estas burbujas dependen de que el número tenga habilitado el servicio de
   * pagos correspondiente (FBPAY/NOVI/UPI) en WhatsApp — el bot solo puede
   * mandar la invitación, no procesar el cobro.
   */
  async handlePaymentInvite(content, _quoted) {
    const data = content.paymentInviteMessage;
    const serviceMap = { FBPAY: 1, NOVI: 2, UPI: 3 };
    const service =
      typeof data.serviceType === "number"
        ? data.serviceType
        : serviceMap[String(data.serviceType).toUpperCase()] ?? 3;
    return {
      paymentInviteMessage: proto.Message.PaymentInviteMessage.fromObject({
        serviceType: service,
        expiryTimestamp:
          data.expiryTimestamp ||
          Math.floor(Date.now() / 1000) + 60 * 60 * 24, // 24h default
        incentiveEligible: !!data.incentiveEligible,
        referralId: data.referralId,
        inviteType: data.inviteType || 0, // DEFAULT
      }),
    };
  }
