...
export const makeMessagesSocket = (config) => {
  ...
  const socketWithMessages = {
    ...
    /**
     * Manda una burbuja de "solicitud de pago" (proto.Message.RequestPaymentMessage).
     * WhatsApp la muestra como un botón "Pagar $X" en el chat.
     *
     * @param {string} jid - JID del chat destino.
     * @param {object} params
     * @param {number} params.amount1000 - Monto en miles (ej. $5.00 -> 5000).
     * @param {string} [params.currency='USD'] - Código ISO 4217.
     * @param {string} [params.from='0@s.whatsapp.net'] - JID de quien debe pagar.
     * @param {string} [params.note] - Texto de la nota que acompaña la solicitud.
     * @param {number} [params.expiry] - Unix timestamp de expiración.
     * @param {object} quoted - Mensaje a responder (opcional).
     * @example
     * await sock.requestPaymentFrom(jid, { amount1000: 5000, currency: "USD", note: "Por el sticker pack" });
     */
    requestPaymentFrom: async (jid, params = {}, quoted) => {
      return socketWithMessages.sendMessage(
        jid,
        {
          requestPaymentMessage: {
            amount: params.amount1000,
    ...
    /**
     * Manda una burbuja de "orden de compra" (proto.Message.OrderMessage) —
     * la burbuja con thumbnail/título/monto y botón "Ver detalles".
     * Nota: el checkout dentro de la burbuja no es controlable por el bot,
     * solo su representación visual (ver dugong.js -> handleOrder).
     *
     * @param {string} jid
     * @param {object} params
     * @param {string} params.orderTitle
     * @param {Buffer} [params.thumbnail]
     * @param {number} [params.itemCount]
     * @param {number} [params.totalAmount1000]
     * @param {string} [params.currency='USD']
     * @param {string} [params.sellerJid]
     * @param {string} [params.message]
     */
    orderText: async (jid, params = {}, quoted) => {
      return socketWithMessages.sendMessage(
        jid,
        {
          orderMessage: {
            ...
            sellerJid: params.sellerJid,
            token: params.token,
            totalAmount1000: params.totalAmount1000,
            currency: params.currency,
    ...
    /**
     * Manda una burbuja de "invitación de pago" (proto.Message.PaymentInviteMessage).
     *
     * @param {string} jid
     * @param {object} params
     * @param {"FBPAY"|"NOVI"|"UPI"|number} [params.serviceType='UPI'] - Servicio de pago.
     * @param {number} [params.expiryTimestamp] - Unix timestamp de expiración (default: +24h).
     * @param {boolean} [params.incentiveEligible]
     * @param {string} [params.referralId]
     */
    sendPaymentInvite: async (jid, params = {}, quoted) => {
      return socketWithMessages.sendMessage(
        jid,
        {
          paymentInviteMessage: {
            ...
            incentiveEligible: params.incentiveEligible,
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      if (
        ...
        switch (messageType) {
          ...
          case "ORDER": {
            const orderContent = await ourin.handleOrder(content, quoted);
            return await relayMessage(jid, orderContent, {
              messageId: generateMessageIDV2(userJid),
            });
          }
          case "PAYMENT_INVITE": {
            const inviteContent = await ourin.handlePaymentInvite(
              content,
              quoted,
            );
            return await relayMessage(jid, inviteContent, {
              messageId: generateMessageIDV2(userJid),
            });
          }
          case "PRODUCT": {
