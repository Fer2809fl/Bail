...
const paint = (text, color, bold = false) => `${bold ? BOLD : ''}${ESC}38;5;${color}m${text}${RESET}`;
const gradientLine = (text, offset = 0) => {
    const chars = [...text];
...
let printed = false;
export const printBanner = (version = BAILEYSX_VERSION) => {
    ...
    const pad = (text, visibleLength = text.length) => {
...
export const printWaVersionNotice = ({ version, source }) => {
    ...
    if (source === 'bundled') {
        console.log(`${paint('⚠', 214)} ${paint(`No se pudo consultar la versión de WA Web: se usa la incluida (${v}).`, 214)}\n  ${paint('Si WhatsApp ya publicó una más nueva, la vinculación fallará ("código incorrecto").', 250)}\n`);
        return;
    }
